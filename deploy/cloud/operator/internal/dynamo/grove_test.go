...
func TestResolveKaiSchedulerQueueName(t *testing.T) {
	tests := []struct {
		...
		{
			name: "whitespace queue name should be preserved",
			annotations: map[string]string{
				commonconsts.KubeAnnotationKaiSchedulerQueue: "  custom-queue  ",
			},
			expected: "  custom-queue  ",
...
func TestDetermineKaiSchedulerQueue(t *testing.T) {
	tests := []struct {
		name          string
		annotations   map[string]string
	...
	}{
		{
			name:          "default queue",
			annotations:   nil,
			expectedQueue: commonconsts.DefaultKaiSchedulerQueue,
			expectedError: true, // Will fail because queue doesn't exist in test
		},
		{
			name: "custom queue",
			annotations: map[string]string{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := DetermineKaiSchedulerQueue(context.Background(), tt.annotations)

			if tt.expectedError {
				if err == nil {
					t.Errorf("expected error but got none")
				}
				// The error is expected because the queue doesn't exist in our test setup
			} else {
				if err != nil {
					t.Errorf("unexpected error: %v", err)
				}
				if result != tt.expectedQueue {
					t.Errorf("expected queue %v, got %v", tt.expectedQueue, result)
				}
			}
		})
	}
